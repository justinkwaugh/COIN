define(function (require) {
    ...
    var Rally = require('fallingsky/commands/rally');
    ...
    var TestTrade = Selfish.Base.extend({
        ...
        run: function run() {
            ...
            PlaceWarbands.perform(state, { faction: aedui, region: mandubiiRegion, count: 2});
            ...
            var executableRallyRegions = bot.getExecutableRallyRegions(state, aedui);
            if(bot.isRallyEffective(executableRallyRegions)) {
                Rally.execute(state, {faction: aedui, regions: _.map(executableRallyRegions, 'region')});
            }
        }
    });
