define(function (require) {
    ...
    var TestRaid = Selfish.Base.extend({
        ...
        run: function run() {
            ...
            PlaceWarbands.perform(state, { faction: aedui, region: provinciaRegion, count: 2});
            ...
            PlaceWarbands.perform(state, { faction: aedui, region: venetiRegion, count: 4});
            PlaceWarbands.perform(state, { faction: belgae, region: venetiRegion, count: 1});

            var bot = AeduiBot.create();
            var effectiveRaids = bot.getEffectiveRaidRegions(state, aedui);
            if(effectiveRaids.length > 0) {
                bot.raid(state, aedui, effectiveRaids);
            }
        }
    });
